        ...
        ?>
            <div class="ferramenta-card">
                <?php if (!empty($f['fotoFerramenta'])): ?>
                    <img src="Img/<?= htmlspecialchars($f['fotoFerramenta']) ?>" alt="<?= htmlspecialchars($f['nomeFerramenta']) ?>">
                <?php else: ?>
                    <img src="Img/placeholder.png" alt="Sem Foto"> 
                <?php endif; ?>

                <h4><?= htmlspecialchars($f['nomeFerramenta']) ?></h4>
                <p>R$ <?= number_format($f['precoFerramenta'], 2, ',', '.') ?> / dia</p>
                
                <a href="?pagina=detalhe_ferramenta&id=<?= $f['idFerramenta'] ?>">
                    Ver Detalhes
                </a>
            </div>
        <?php
